...
export default function Archive() {
    ...
    const test = () =>{
        abc=document.getElementById("car")
        lol=abc.value
        comma=lol.indexOf(',')
        var x;
        var y;
        
        x=Number(lol.slice(0,comma))
        console.log(x)
        console.log(typeof(x))
        y=Number(lol.slice(comma+1,lol.length))
        console.log(y)
    ...
    return (
        <>
            ...
            <option value={[28.6513731,77.2110377]}>1</option>
            </select>
            <input type="submit" value="Submit" className="btn" onClick={test} />
        </>
    )
}